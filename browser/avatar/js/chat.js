...
function connectAvatar() {
  ...
  const cogSvcSubKey = "7d78c805a6a9499e97820bbdbcf47df0";
  if (cogSvcSubKey === "") {
  ...
  // const privateEndpointEnabled = "https://ai-yogarohith1858ai878864920350.cognitiveservices.azure.com/"
  const privateEndpoint = document
    .getElementById("privateEndpoint")
    .value.slice(8);
  // const privateEndpoint ="https://ai-yogarohith1858ai878864920350.cognitiveservices.azure.com/"
  ...
  avatarConfig.useBuiltInVoice =
    document.getElementById("useBuiltInVoice").checked;
    ...
    peerConnectionDataChannel.onmessage = (e) => {
      ...
      if (
        ...
        if (webRTCEvent.event.eventType === "EVENT_TYPE_SESSION_END") {
          if (
            document.getElementById("autoReconnectAvatar").checked &&
            !userClosedSession &&
            !isReconnecting
          ) {
            ...
            if (new Date() - lastInteractionTime < 300000) {
              ...
              console.log(
                `[${new Date().toISOString()}] The WebSockets got disconnected, need reconnect.`,
              );
              isReconnecting = true;
...
function handleUserQuery(userQuery, userQueryHTML, imgUrlPath) {
  ...
  const azureOpenAIEndpoint = document.getElementById(
    "azureOpenAIEndpoint",
  ).value;
  const azureOpenAIApiKey = document.getElementById("azureOpenAIApiKey").value;
  const azureOpenAIDeploymentName = document.getElementById(
    "azureOpenAIDeploymentName",
  ).value;
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        userMessage.innerHTML +=
          '<br/><img id="picInput" src="https://wallpaperaccess.com/full/528436.jpg" style="width:100px;height:100px"/><br/><br/>';
      });
  } else {
    document.getElementById("userMessageBox").hidden = true;
    document.getElementById("uploadImgIcon").hidden = true;
...
//   console.log("Autofilling Azure Config and starting session...");

//   // Pre-fill Azure Speech resource config
//   document.getElementById("region").value = "westeurope";
...
//   // Auto-click the “Open Avatar Session” button
//   setTimeout(() => {
...
//     } else {
//       console.warn("startSession button not found.");
//     }
//   }, 1000); // Wait 1 second to ensure DOM and SDK are ready
// });
// window.addEventListener("DOMContentLoaded", () => {
//   console.log("Auto-configuring Azure settings...");

//   // ✅ Set Region
//   const regionSelect = document.getElementById("region");
//   if (regionSelect) regionSelect.value = "westeurope";

//   // ✅ Azure Speech API Key
//   const speechApiKey = document.getElementById("APIKey");
//   if (speechApiKey) speechApiKey.value = "7d78c805a6a9499e97820bbdbcf47df0";

//   // ✅ Azure OpenAI Config
//   const openAIEndpoint = document.getElementById("azureOpenAIEndpoint");
...
//   // ✅ Automatically click "Open Avatar Session"
//   setTimeout(() => {
//     const startBtn = document.getElementById("startSession");
//     if (startBtn) {
//       startBtn.click();
//       console.log("✅ Avatar session started automatically.");
//     } else {
//       console.warn("❌ Could not find 'startSession' button.");
//     }
//   }, 1000); // Wait for full render
// });
...
//     if (startBtn) {
//       startBtn.click();
//       console.log("✅ Clicked startSession button.");
//     } else {
//       console.warn("❌ 'startSession' button not found.");
//     }
//   };

//   // ✅ Reload iframe if session fails to start within timeout
//   const monitorAvatarStart = () => {
//     let avatarStarted = false;

//     // Hook into a known global like `speechSynthesizer` or add your own flag after success
//     const observer = new MutationObserver(() => {
//       // Check for presence of avatar video, element, or change
//       const avatarEl = document.querySelector("canvas, video, .avatar-container");
//       if (avatarEl) {
//         avatarStarted = true;
//         console.log("✅ Avatar appears to have started.");
//         observer.disconnect();
//       }
//     });
...
//     setTimeout(() => {
//       if (!avatarStarted) {
//         console.error("❌ Avatar did not start. Reloading iframe...");
//         // Reload iframe from parent
//         if (window !== window.parent) {
//           window.parent.postMessage({ type: "reloadIframe" }, "*");
//         } else {
//           location.reload(); // Fallback if not in iframe
//         }
//       }
//     }, 7000); // wait 7 seconds before deciding it failed
//   };
...
// });
window.addEventListener("DOMContentLoaded", () => {
  console.log("🟢 Auto-clicking 'Open Avatar Session'...");

  setTimeout(() => {
    const startBtn = document.getElementById("startSession");
    if (startBtn) {
      startBtn.click();
      console.log("✅ Clicked 'Open Avatar Session' button.");
    } else {
      console.warn("❌ 'Open Avatar Session' button not found.");
    }
